...
class BrowserApp {
    ...
    private setupNavigationControls() {
        ...
        backBtn?.addEventListener('click', () => {
            if (this.currentTabId) window.electronAPI.goBack(this.currentTabId);
        });
        forwardBtn?.addEventListener('click', () => {
            if (this.currentTabId) window.electronAPI.goForward(this.currentTabId);
        });
    ...
    private initQuickToggle(elementId: string, settingKey: string) {
        const el = document.getElementById(elementId);
        if (!el) return;
        if (this.settings[settingKey]) el.classList.add('active');
    ...
    private setupFullSettingsPanel() {
        // Close button
        ...
        document.getElementById('settings-overlay')?.addEventListener('click', (e) => {
            if (e.target === e.currentTarget) this.closeSettingsPanel();
        });

        // Keyboard shortcut Escape
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') this.closeSettingsPanel();
        });

        // Sidebar section navigation
        document.querySelectorAll<HTMLButtonElement>('.settings-nav-item[data-section]').forEach(btn => {
        ...
        // Homepage
        const homepageInput = document.getElementById('setting-homepage') as HTMLInputElement;
        ...
        document.getElementById('btn-pick-download')?.addEventListener('click', async () => {
            ...
            if (folder) {
                ...
                const input = document.getElementById('setting-downloadPath') as HTMLInputElement;
    ...
    private populateSettingsPanel() {
        ...
        // Homepage
        const homepageInput = document.getElementById('setting-homepage') as HTMLInputElement;
        if (homepageInput) homepageInput.value = s.homepage || '';
        ...
        // Download path
        const dlInput = document.getElementById('setting-downloadPath') as HTMLInputElement;
        if (dlInput) dlInput.value = s.downloadPath || '';

        // Zoom
        const zoomDisplay = document.getElementById('zoom-display');
        if (zoomDisplay) zoomDisplay.textContent = `${s.defaultZoom || 100}%`;
        ...
        if (adsToggle) adsToggle.classList.toggle('active', !!s.blockAds);
        if (httpsToggle) httpsToggle.classList.toggle('active', !!s.forceHttps);
    }
    ...
    private setupRadioGroup(settingKey: string) {
        document.querySelectorAll<HTMLInputElement>(`input[name="${settingKey}"]`).forEach(radio => {
    ...
    private setPanelToggleState(elementId: string, value: boolean) {
        const el = document.getElementById(elementId);
        if (el) el.classList.toggle('active', !!value);
    }

    private setupPanelToggle(elementId: string, settingKey: string) {
        const el = document.getElementById(elementId);
        if (!el) return;
        el.addEventListener('click', async () => {
            el.classList.toggle('active');
